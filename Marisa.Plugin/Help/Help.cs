...
public class Help : MarisaPluginBase
{
    ...
    private static List<HelpDoc>? _helpDocs;

    [MarisaPluginCommand]
    private static MarisaPluginTaskState Handler(Message message, IEnumerable<MarisaPluginBase> plugins)
    {
        _helpDocs ??= GetHelp(plugins);

        var bm    = HelpDoc.DrawHelpList(_helpDocs)!;
        ...
        message.Reply(MessageDataImage.FromBase64(bmRes.ToB64()));
