    ...
    private static Func<Song, bool> GetConstraint(Message message, List<ReadOnlyMemory<char>> expr)
    {
        ...
        foreach (var e in expr)
        {
            ...
            switch (available.FindIndex(x => x.StartsWith(key, StringComparison.InvariantCultureIgnoreCase)))
            {
                ...
                case 0: // Charter
                    CheckEqOp(op);
                    diffConstraint.Add((song, levelIdx) => MakeEqConstraint(op, song.Charters[levelIdx], val));
                    break;
                case 1: // Constant
                ...
                case 3: // Artist
                    CheckEqOp(op);
                    songConstraint.Add(song => MakeEqConstraint(op, song.Artist, val));
                    break;
                case 4: // Title
                    CheckEqOp(op);
                    songConstraint.Add(song => MakeEqConstraint(op, song.Title, val));
                    break;
                case 5: // Version
                    CheckEqOp(op);
                    songConstraint.Add(song => MakeEqConstraint(op, song.Version, val));
                    break;
                case 6: // Id
                ...
                case 8: // DiffName
                    CheckEqOp(op);
                    diffConstraint.Add((song, levelIdx) => MakeEqConstraint(op, song.DiffNames[levelIdx], val));
                    break;
                case 9: // Level
