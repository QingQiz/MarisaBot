...
public static class MaiMaiDraw
{
    ...
    public static Image DrawB40Card(DxRating rating)
    {
        Image GetScoreCard(SongScore score)
        {
            ...
            card.Mutate(i => i
                // DX or SD
                .DrawImage(ResourceManager.GetImage(score.Type == "DX" ? "type_deluxe.png" : "type_standard.png"), 0, 0)
                // FC 标志
                .DrawImage(fcImg, 125, 0)
                // FS 标志
                .DrawImage(fsImg, 160, 0)
            );
            ...
            coverBackground.Mutate(i => i
                ...
                .Fill(MaiMaiSong.LevelColor[score.LevelIdx], new RectangleF(levelBar.PaddingLeft, levelBar.PaddingTop, levelBar.Width, levelBar.Height))
            );
            ...
            var achievement = score.Achievement.ToString("000.0000").Split('.');
            ...
            coverBackground.Mutate(i => i
                ...
                // 达成率整数部分
                .DrawText(achievement[0], new Font(contentFontFamily, 56), fontColor, 25, 52)
                // 达成率小数部分
                .DrawText("." + achievement[1], contentFontFamily, 37, fontColor, 115, 65)
                // rank 标志
                ...
                .DrawText(score.Rating.ToString("000"), ratingFont, fontColor, 175, 115)
                // card
                .DrawImage(card, 25, 155)
            );
        ...
        {
            const int column = 5;
            var       row    = (rating.OldScores.Count + column - 1) / column + (rating.NewScores.Count + column - 1) / column;
            ...
            for (var i = 0; i < rating.OldScores.Count; i++)
            {
                background.DrawImage(GetScoreCard(rating.OldScores[i]), px, py);
            ...
            var sdRowCount = (rating.OldScores.Count + column - 1) / column;
            pxInit = paddingH;
            pyInit = cardHeight * sdRowCount + paddingV * (sdRowCount + 1 + 3);

            background.Mutate(i => i
                // ReSharper disable once PossibleLossOfFraction
                .Fill(Color.FromRgb(120, 136, 136), new RectangleF(paddingH, pyInit - 2 * paddingV, bgWidth - 2 * paddingH, paddingV / 2))
            );

            px = pxInit;
            py = pyInit;

            for (var i = 0; i < rating.NewScores.Count; i++)
            {
                background.DrawImage(GetScoreCard(rating.NewScores[i]), px, py);
        ...
        Image GetRatingCard()
        {
            var (dxRating, sdRating) = (rating.NewScores.Sum(s => s.Rating), rating.OldScores.Sum(s => s.Rating));

            var addRating = rating.B50 ? 0 : rating.AdditionalRating;
            ...
            // rating 部分
            var ratingCard = ResourceManager.GetImage($"rating_{num}.png").Resize(2);
            ...
            {
                if (ra[i] == ' ') break;
                ratingCard.DrawImage(ResourceManager.GetImage($"num_{ra[i]}.png"), 170 + 29 * i, 20);
            }

            ratingCard = ratingCard.Resize(1.4);

            // 名字
            var nameCard = new Image<Rgba32>(690, 140);

            nameCard.Clear(Color.White);
            ...
            while (true)
            {
                var w = rating.Nickname.Measure(font);

                if (w.Width < 480)
                {
                    nameCard.DrawText(rating.Nickname, font, Color.Black, 20, (nameCard.Height - w.Height) / 2);
                    break;
                }
            ...
            var dx = ResourceManager.GetImage("icon_dx.png", 3.2);

            nameCard.Mutate(i => i.DrawImage(dx, 500, 10).RoundCorners(20));

            // 称号（显示底分和段位）
            var rainbowCard = ResourceManager.GetImage("rainbow.png");
            rainbowCard.Resize((double)nameCard.Width / rainbowCard.Width + 0.05);
            ...
            var userInfoCard = new Image<Rgba32>(nameCard.Width + 6, ratingCard.Height + nameCard.Height + rainbowCard.Height + 20);

            userInfoCard.DrawImage(ratingCard, 0, 0)
                .DrawImage(nameCard, 3, ratingCard.Height + 10)
                .DrawImage(rainbowCard, 0, ratingCard.Height + nameCard.Height + 20);
            ...
            background.DrawImage(dlx, 0, 20).DrawImage(userInfoCard, userInfoCard.Height + 10, 20);
